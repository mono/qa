...
class Driver(object):
    ...
    def __init__(self, username, password, 
                ...
                resource="tr_xmlrpc.cgi", port=None, ssl=False):
        ...
        self.server = xmlrpclib.ServerProxy("http" + ("", "s")[ssl] +\
                    "://%s:%s@%s%s/%s" % (username, password, host, (":%s" % port, "")[port==None], resource), verbose = VERBOSE)
    ...
    def do_command(self, verb, args):
        ...
        try:
            ...
            #   % (str(e.args),e.errcode, e.errmsg, str(e.headers), e.message, e.url)
    ...
    def testcase_list(self, case_id=None, case_id_type=None,
        ...
        return self.do_command("TestCase.list", [self._options_ne_dict(
                   ...
                   self._list_dict_op('plans', plans),self._number_option('plan_id', plan_id),
    ...
    def user_look_up_id(self, username):
        """Get the id of an existing user
        PARAMETERS
        username        (str)    The name of the testopia user

        RETURNS
        (int) The testopia id of the user
        
        EXAMPLES
        user_look_up_id('jandersen')
        """
        return self.do_command("User.lookup_id_by_login ", [self._string_noop(username)])
