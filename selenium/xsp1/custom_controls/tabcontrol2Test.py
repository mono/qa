...
class CustomControls_tabcontrolTest(seleniumTestCase):
    ...
    def test(self):
        ...
        sel.click("tabs_btn")
        sel.wait_for_page_to_load("30000")
        try: self.failUnless(re.search(r"^[\s\S]*pressed me .* time[\s\S]*$", sel.get_text("//*[@id=\"tabs_uno\"]")))
        except AssertionError, e: self.verificationErrors.append(str(e))
        sel.click("tabs_btn")
        sel.wait_for_page_to_load("30000")
        try: self.failUnless(re.search(r"^[\s\S]*pressed me .* times[\s\S]*$", sel.get_text("//*[@id=\"tabs_uno\"]")))
        except AssertionError, e: self.verificationErrors.append(str(e))
